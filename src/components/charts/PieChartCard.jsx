...
/**
 * PieChartCard — reusable full pie chart with configurable legend layout
 *
 * Props:
 *   title          {string}   required
 *   data           {Array}    required — [{ name, value }]
 *   colors         {Array}    default purple palette
 *   legendCols     {number}   1 = single column (default), 2 = 2-column grid
...
export default function PieChartCard({
  ...
  const legendClass = legendCols === 2
    ? "grid grid-cols-2 gap-x-4 gap-y-3 flex-1 min-w-0"
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 flex-1 min-h-0">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Tooltip
                ...
                contentStyle={{ fontSize: "11px", borderRadius: "6px", border: "1px solid #e6e8f0" }}
              />
              <Pie
                data={data}
                dataKey="value"
        ...
        {/* LEGEND — layout controlled by legendCols prop */}
        <div className={legendClass}>
          {data.map((item, i) => (
            <div key={i} className="flex flex-col gap-0.5">
              ...
              <span className="text-[11px] font-bold text-gray-800 pl-4">
                {format(item)}
              </span>
            </div>
          ))}
